...
func NewApp(shutdown chan os.Signal, mw ...Middleware) *App {
	...
	// Support for Options method for CORS preflight.
	mux.HeadCanUseGet = true
	mux.OptionsHandler = func(w http.ResponseWriter, r *http.Request, params map[string]string) {
		// Handle CORS preflight request in the middleware.
...
func (a *App) EnableCORS(cm Middleware) {
	...
	a.ContextMux.OptionsHandler = func(w http.ResponseWriter, r *http.Request, _ map[string]string) {
		handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
			return Respond(ctx, w, "OK", http.StatusOK)
		}
		h := a.newHandlerFunc(handler)
...
func (a *App) Handle(method string, group string, path string, handler Handler, mw ...Middleware) {
	...
	a.newRoute(method, group, path, a.newHandlerFunc(handler))
}

func (a *App) newHandlerFunc(handler Handler) http.HandlerFunc {
...
func (a *App) newRoute(method string, group string, path string, handler http.HandlerFunc) {
	finalPath := path
	if group != "" {
		finalPath = "/" + group + path
	}

	fmt.Println("Registering route", method, finalPath)
	a.ContextMux.Handle(method, finalPath, handler)
}
